...
- **Predictions from a trained, tested model, not a guess.**
  ...
  - **Validation without leakage:** samples are taken weekly. The most recent 20% of dates with a known outcome
    are held out, and training uses only dates whose outcome was known before that period started.
  ...
  - **Track record:** one run a week keeps its predictions. When their 30 days are up, the next run scores them
    against actual prices and drops the rows. Other runs' rows are dropped once a newer run publishes, which keeps
    the table small.
